...
class CollectionReadCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('youppers:product:collection:read')
            ->setDescription('Show info about a Product Collection')
            ->addOption('sessionId', 'i', InputOption::VALUE_OPTIONAL, 'Session id', null)
			->addOption('group', 'g', InputOption::VALUE_OPTIONAL, 'serialization group','json, json.collection.read')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $collection = $productService->readCollection($input->getArgument('collectionId'));
        $output->writeln($this->serialize($collection, $input->getOption('group')));        
    }
}